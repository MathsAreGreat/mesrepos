...
def default_site_config(apps, schema_editor):
    ...
    Config.objects.bulk_create(
        [
            ...
            Config(key="school_address", value="Taroudant, Morocco"),
        ]
    )
    ...
    Session.objects.bulk_create(
        [
            Session(name="2025/2026", current=True),
        ]
    )
    ...
    Subject.objects.bulk_create(
        [
            ...
            Subject(name="Sports"),
            Subject(name="Pysics & Chemistry"),
            Subject(name="History & Geography"),
        ]
    )

    StudentClass = apps.get_model("corecode", "StudentClass")
    StudentClass.objects.bulk_create(
        [StudentClass(name=f"{i}APIC-{j}") for i in range(1, 4) for j in range(1, 14)]
    )
