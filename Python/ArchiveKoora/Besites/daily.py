...
from Besites.variables import DOWN_PATH
...
def check_for_datas(k):
    doc = Path("datas/Daily")
    fn = doc / k
    if Path(fn).exists():
        return None
    ok_url = f"https://www.dailymotion.com/video/{k}"
    r = sess.get(ok_url)
    if "<h1" not in r.text:
...
def getUrl(k, t):
    ...
    for e in data["formats"]:
        if e.get("protocol") and e["protocol"] == "m3u8_native":
            ...
            pl = get_playlist(uri)
            dts = get_segments(pl)
            return [(u, t, f"{i:06}.ts") for i, u in enumerate(dts, start=1)]
...
def get_playlist(uri):
    ...
    for playlist in mdt.playlists:
        _, h = playlist.stream_info.resolution
        if h > 1500:
            continue
...
def sauver(k, t, f=False):
    d = datetime.now()
    n = d.strftime("%Y%m%d%H%M")
    doc = f"datas/DAILY/{n}"
    fjson = f"{doc}/{k}.json"
    if os.path.exists(fjson) and not f:
        with open(fjson, "r") as e:
            sgs = json.load(e)
    else:
...
if __name__ == "__main__":
    ok_url = "https://dai.ly/x9hd5cs"
    fjson = "/home/mohamed/Documents/Projects/Python/ArchiveKoora/tx9hd5cs.json"
